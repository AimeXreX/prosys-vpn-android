...
object Utils {
    ...
    private fun isIpv6Address(value: String): Boolean {
        var addr = value
        if (addr.startsWith("[")) {
            val bracketEnd = addr.indexOf(']')
            if (bracketEnd <= 1) return false
            val suffix = addr.substring(bracketEnd + 1)
            if (suffix.isNotEmpty() && !suffix.matches(Regex(":\\d+"))) return false
    ...
    fun isIpInCidr(ip: String, cidr: String): Boolean {
        try {
            ...
            // Logging must never change this validator's false-on-invalid-input contract.
            runCatching { LogUtil.e(AppConfig.TAG, "Failed to check if IP is in CIDR", e) }
            return false
        }
    }
